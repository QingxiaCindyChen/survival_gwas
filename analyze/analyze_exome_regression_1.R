...
logFilepath = file.path(resultDir, sprintf('%s_progress_cox_1.txt', filePrefix))
...
phenoLogistic = Reduce(function(...) merge(..., all = TRUE), phenoLogisticList)
phenoLogistic[is.na(phenoLogistic)] = -9

colnames(phenoLogistic)[1] = 'FID'
phenoLogistic[, IID := FID]
setcolorder(phenoLogistic, c(1, ncol(phenoLogistic)))

phenoFilepath = tempfile('pheno_', fileext = '.tsv')
write_tsv(phenoLogistic, phenoFilepath)

plinkArgs = sprintf('%s --bfile %s --extract %s --covar %s --pheno %s --1 --memory %d',
						  '--logistic hide-covar beta --ci 0.95', file.path(genoDir, genoPrefix),
						  snpFilepath, covarFilepath, phenoFilepath, plinkMemSize)

logFilepath = file.path(resultDir, sprintf('%s_progress_logistic_1.txt', filePrefix))
timeStarted = Sys.time()
cat(sprintf('%s started analysis\n', timeStarted), file = logFilepath)
...
save(list = setdiff(ls(), 'genoData'),
	  file = file.path(resultDir, sprintf('%s_workspace_1.Rdata', filePrefix)))
