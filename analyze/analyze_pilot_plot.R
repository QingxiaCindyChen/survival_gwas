...
coxDf = foreach(phenotypeName=phenotypeNames, .combine=rbind) %do% {
	read_csv(file.path(resultFolder, sprintf('surv1_%s.csv', phenotypeName)), col_types=cols()) %>%
...
glmDf = foreach(phenotypeName=phenotypeNames, .combine=rbind) %do% {
	read_csv(file.path(resultFolder, sprintf('logistic2_%s.csv', phenotypeName)), col_types=cols()) %>%
		mutate(phenotypeName = phenotypeName)}

a = inner_join(glmDf, snpInfo, by='snpName')

# for (phenotypeNow in phenotypeNames) {
# 	pdf(file.path(resultFolder, sprintf('logistic2_man_%s.pdf', phenotypeNow)), width=6, height=4)
# 	manhattan(a %>% filter(phenotypeName==phenotypeNow), p='pval', snp='snpName', chr='chr', bp='pos',
# 				 main = phenotypeNow)
# 	dev.off()
# }
#
# for (phenotypeNow in phenotypeNames) {
# 	pdf(file.path(resultFolder, sprintf('logistic2_qq_%s.pdf', phenotypeNow)), width=4, height=4)
# 	qq(a %>% filter(phenotypeName==phenotypeNow) %>% .$pval, main = phenotypeNow)
# 	dev.off()
# }
...
#
# ggsave(file.path(resultFolder, 'compare_pval_surv1_logistic2.pdf'), plot=p, width=8, height=5.75)
