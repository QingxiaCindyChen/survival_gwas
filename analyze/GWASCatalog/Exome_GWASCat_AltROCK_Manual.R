...
GWASCat = setDT(read_delim(file = file.path(filepath, 'GWASCatalog', 'catalog_phecode_map.txt'), delim = '\t', col_types = cols()))
...
FilePhecodes = FilePhecodes[FilePhecodes %in% MatchPhes] #82
...
#Build concatenated table of all pvals and GWAS Cat calls across all phecodes
ROC = NULL
...
SurvDataCox = ROC[ROC$method=='Cox', ]
#NAs in Logistic
SurvDataLog = ROC[ROC$method=='Logistic', ]
...
OutROC = Output + annotate("text", x = 0.782, y = 0.1, label = paste("Cox AUC = ", round(CoxAUC, 3)), size = 3)+
  annotate("text", x = 0.763, y = 0.05, label = paste("Logistic AUC = ", round(LogAUC, 3)), size = 3)

ggsave(filename = 'MultiPhecodesROCLogScale.pdf', path = file.path(filepath, 'GWASCatalog'), plot = OutROC, width = 5, height = 3.5, units = c('in'))
